...
try:
    ...
    for evt_id, eq_id in anciens_evts:
        # On vérifie si le lien existe déjà pour ne pas faire de doublon
    ...
    pass # Sécurité : si la table n'existe pas encore (ex: première installation), on ignore silencieusement
...
# --- MIGRATION : Lier l'agenda aux événements pour le Magic Link ---
try:
    c.execute("ALTER TABLE agenda ADD COLUMN evenement_id INTEGER")
    commit_and_sync()
except Exception:
    pass # La colonne existe déjà, on ignore l'erreur
